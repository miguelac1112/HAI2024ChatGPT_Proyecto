...
let cover = document.querySelector('.cover'); // Seleccionar la clase de la cubierta

let stop_recognition = () =>{
    speechRecognition.stop_recognition();
    buttonRecognition.style.background = "#38e08c";
    buttonRecognition.innerText = "PRESIONA PARA HABLAR"; // Cambio del texto del botón
...
buttonRecognition.onmousedown = () => {
    if(!recognition_started){
        ...
        cover.classList.add('pulse'); // Agregar la clase para animar la cubierta
    } else{
        stop_recognition();
    }
}
